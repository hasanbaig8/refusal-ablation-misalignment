    ...
    for X, y in tqdm(loader):
        ...
        with torch.no_grad():
            with llm.trace(X) as tracer:
                chosen_seq_idxs = torch.arange(-look_back, 0).repeat(len(seq_idxs),1) + torch.tensor(seq_idxs).unsqueeze(1)
                activations = torch.stack(
                    [layer.output[0][torch.arange(len(seq_idxs)).unsqueeze(1), chosen_seq_idxs, :] for layer in llm.model.layers]
...
def generate_val(llm,val_list, save = False, save_suffix = ''):
    ...
    for X, y in tqdm(harmful_val_loader):
        ...
        X_ids = llm.tokenizer(X, return_tensors='pt', padding=True)
        X_ids = {k: v.to(llm.device) for k, v in X_ids.items()}
        generations = llm.generate(**X_ids, max_length=100)
        text_generations = [llm.tokenizer.decode(generation) for generation in generations]
